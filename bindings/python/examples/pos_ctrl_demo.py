...
def main() -> None:
    p = argparse.ArgumentParser(description="One-shot target position control (POS_VEL mode)")
    p.add_argument("--channel", default="can0")
    p.add_argument("--model", default="4310")
    p.add_argument("--motor-id", default="0x01")
    p.add_argument("--feedback-id", default="0x11")
    p.add_argument("--target-pos", type=float, required=True)
    p.add_argument("--vlim", type=float, default=1.5)
    p.add_argument("--loop", type=int, default=300)
    ...
    print(
        ...
        f"feedback_id=0x{feedback_id:X} mode=pos-vel target_pos={args.target_pos} vlim={args.vlim}"
    )
    ...
    with Controller(args.channel) as ctrl:
        ...
        try:
            ...
            for i in range(args.loop):
                m.send_pos_vel(args.target_pos, args.vlim)
                if args.print_state:
                    st = m.get_state()
                    if st is None:
