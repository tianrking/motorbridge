...
<Badge variant="primary">v0.1.3</Badge> <Badge variant="secondary">Rust 核心</Badge> <Badge variant="outline">Python 绑定</Badge>
...
<Warning>
每个 Controller 绑定**一个厂商**。你不能在同一个 Controller 实例中混合使用达妙和 RobStride 电机。
</Warning>
...
```bash
cargo run -p ws_gateway -- --bind 0.0.0.0:9002
```
