...
| MyActuator | X 系列（运行时型号字符串，默认 `X8`） | enable、disable、stop、status、current、vel、pos、version、mode-query | 暂无（CLI 命令级支持） | 不支持 | 使用标准 11-bit ID：`0x140+id` / `0x240+id`；常用 ID 范围 1..32 |
